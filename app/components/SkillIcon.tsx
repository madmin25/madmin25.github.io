...
export default function SkillIcon({iconSize=160, strokeWidth=20, title="", level=50, src=""}: SkillIconProps) {
    ...
    <div className='flex flex-col justify-center items-center   ' style={{width: test , height: test, minWidth: test, minHeight: test}}>
        <div className='flex w-[calc(100%)] h-[calc(100%)] items-center justify-center rounded-full'>
                ...
                <svg className='absolute top-0 left-0 z-10 -rotate-90' style={{width: test , height: test}}>
                    ...
                    <circle className='sliderOff fill-transparent' r={radius} cx={circleOffset} cy={circleOffset} style={{ ["--dashOffset" as string]: strokeArray, ["--level" as string]: levelAdjust, stroke: "url(#TalentGradient)", strokeDasharray: strokeArray, strokeDashoffset: strokeArray, strokeWidth: strokeWidth}}></circle>
                </svg>
            
                <svg className='absolute top-0 left-0 w-full h-full '>
                    <circle className='fill-transparent' r={radius} cx={circleOffset} cy={circleOffset} style={{stroke: "#1d7155", strokeWidth: strokeWidth}}></circle>
                </svg>
